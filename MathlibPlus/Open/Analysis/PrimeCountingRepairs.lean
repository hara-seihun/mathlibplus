...
/-- One row of the concrete 28-row audit table supplied with Claim 976. -/
...
/-- The exact 28 rows supplied with Claim 976. Decimal entries are exact rationals. -/
def auditRows : List AuditRow :=
...
/-- The usual real extension of the prime-counting function. -/
def primeCountingReal (x : ℝ) : ℝ :=
  (Nat.primeCounting (Nat.floor x) : ℝ)

/-- The exact sharp coefficient recorded for an audit row. -/
def sharpCoefficient (r : AuditRow) : ℝ :=
  Real.log (r.optimizer : ℝ) - (r.optimizer : ℝ) / (r.piOptimizer : ℝ)

/-- A coefficient gives the strict bound on the row's published range. -/
def strictBound (r : AuditRow) (c : ℝ) : Prop :=
  ∀ x : ℝ, (r.start : ℝ) ≤ x →
    primeCountingReal x < x / (Real.log x - c)

/-- The rowwise content of Claim 980. -/
def validAuditRow (r : AuditRow) : Prop :=
  sharpCoefficient r < r.repair ∧
    r.repair < r.publishedCoeff ∧
    strictBound r r.repair ∧
    strictBound r r.publishedCoeff

/-- Claim 980: every concrete audit row has a valid nine-decimal repair and
    published coefficient, with the same published start. -/
def allPublishedCoefficientsAndRepairsAreValid : Prop :=
  auditRows.length = 28 ∧ ∀ r ∈ auditRows, validAuditRow r
