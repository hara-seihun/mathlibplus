...
namespace MathlibPlus.Open.Geometry.PlanarRhombicCore33947
...
def minimumDiameter (n : ℕ) : ℝ :=
  sInf {d : ℝ | ∃ Y : Configuration n,
    unitSeparated Y ∧ maximumDistance Y = d}

def globalDiameterMinimizer {n : ℕ} (X : Configuration n) : Prop :=
  normalized X ∧ maximumDistance X = minimumDiameter n ∧
    ∀ Y : Configuration n, unitSeparated Y →
      maximumDistance X ≤ maximumDistance Y

def firstOrderIsolatedGlobalDiameterMinimizer {n : ℕ}
    (X : Configuration n) : Prop :=
  firstOrderIsolatedLocalRatioMinimizer X ∧ globalDiameterMinimizer X
...
/-- Claim 33947: a normalized first-order isolated global minimizer with a
triangle-free unit graph has the stated boundary-order rhombic core. -/
def claim33947_linear_rhombic_core : Prop :=
  ∀ (n f : ℕ) (X : Configuration n)
    ...
    hullCount X ≤ hullBound n ∧
      2 * n - 2 ≤ contactCount X + hullCount X ∧
      2 * n - 2 ≤ contactCount X + hullBound n ∧
      ...
      ∀ F : Fin f, isQuadrilateralFace faces outer F →
        ∀ cycle : List (Dart n), cycle ∈ faces F →
          (cycleVertices cycle).Nodup ∧ exactUnitRhombusCycle X cycle

end
end MathlibPlus.Open.Geometry.PlanarRhombicCore33947
