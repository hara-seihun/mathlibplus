...
/-- Claim 39335: singleton exponents are recovered by the displayed folded
identity, and only up to the involution `b ↦ B-2-b`. -/
def claim39335 : Prop :=
  ∀ A : Arm → ℕ,
    Polynomial.X ^ 2 * singletonPolynomial A +
        reciprocalShift (totalExponent A) (singletonPolynomial A) =
      GPolynomial A + eTwo A ∧
    (∀ A' : Arm → ℕ,
      KPolynomial A = KPolynomial A' →
        foldedSingletonExponents A = foldedSingletonExponents A')
