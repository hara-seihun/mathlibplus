...
namespace MathlibPlus.Open.ResearchFormalization.R0761Claim24551
...
def claim24551 : Prop :=
  (∀ p : ℕ, Nat.Prime p → 5 ≤ p →
    ∀ (φ : LiftDih p ≃ LiftDih p) (P : AffineParameters p),
      ...
      P.tP = 0 ∧
        Set.image (affineKernelMap P) baseBSet = baseBSet ∧
        Finset.sum baseB (fun b => b) = vAnchor ∧
        Finset.sum baseB (affineKernelMap P) = vAnchor ∧
        (3 : ℕ) • P.tV = 0 ∧
        P.M vAnchor = vAnchor) ∧
  (∀ p : ℕ, Nat.Prime p → 5 ≤ p →
    ¬ ∃ (φ : LiftDih p ≃ LiftDih p) (P : AffineParameters p),
...
end MathlibPlus.Open.ResearchFormalization.R0761Claim24551
