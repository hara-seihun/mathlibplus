...
namespace MathlibPlus.Open.ResearchFormalization.R1144Claims30196_30204Repair
...
def scalarLabel (δ : C7 → C7) : Prop :=
  ∃ a : C7ˣ, ∀ t : C7, δ t = (a : C7) * t

def pointLabels (π : Equiv.Perm C7) : Set (C7 → C7) :=
  {δ | ∃ F : Set (Set C7),
    fanoSupportSystem π F ∧
      ∃ B : Set C7, B ∈ F ∧ normalizedLabel π B δ}

def nonlinearLabels : Set (C7 → C7) :=
  {δ | (∃ π : Equiv.Perm C7,
...
def complementaryLabelRelation
    (π : Equiv.Perm C7) (B : Set C7) (δ : C7 → C7)
    (t s : C7) : Prop :=
...
source-target classes with transported relative labels and uniform 84-label,
49-fold incidence. -/
...
end MathlibPlus.Open.ResearchFormalization.R1144Claims30196_30204Repair
