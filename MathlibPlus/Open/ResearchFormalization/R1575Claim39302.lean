...
namespace MathlibPlus.Open.ResearchFormalization.R1575Claim39302
...
def tauModU (tau : H → W) (h : H) : F7 := (tau h).2

/-- Claim 39302: the two nonsaturated stabilizer branches force the exact
quotient and restriction cocycle rows. -/
def claim_39302 : Prop :=
  ∀ (lam : H → F7) (tau : H → W) (h : H),
    normalizedProfile lam tau →
      h ∈ leftPeriod lam →
        ...
        (T = translationU →
          (∀ k : H, (periodGenerator lam tau h k).2.2 = 0) ∧
            (∀ k : H,
              tauModU tau (hMul h k) =
                tauModU tau h + chi h * tauModU tau k)) ∧
        (T = {wZero} →
          (∀ g : HeisenbergPoint,
            g ∈ Gamma → g.1 = 0 → g = heisOne) ∧
            (∀ k : H, k ∈ leftPeriod lam →
              defect tau h k = wZero) ∧
              (∀ q : H, q ∈ leftPeriod lam →
                ∀ k : H, k ∈ leftPeriod lam →
                  tau (hMul q k) = tau q + chi q • tau k))

end

end MathlibPlus.Open.ResearchFormalization.R1575Claim39302
