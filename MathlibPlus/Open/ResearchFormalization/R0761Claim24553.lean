...
namespace MathlibPlus.Open.ResearchFormalization.R0761Claim24553
...
def claim24553 : Prop :=
  ∀ p : ℕ, Nat.Prime p → 5 ≤ p →
    inverseClosed (liftedS p) ∧ identityFree (liftedS p) ∧
      inverseClosed (liftedT p) ∧ identityFree (liftedT p) ∧
      cayleyGraphIsomorphism (liftedS p) (liftedT p) ∧
      (¬ ∃ φ : LiftDih p ≃ LiftDih p,
        isDihAutomorphism φ ∧ mapsConnectionSet φ (liftedS p) (liftedT p)) ∧
      ¬ undirectedCIGroup (A := V × ZMod p)
end

end MathlibPlus.Open.ResearchFormalization.R0761Claim24553
