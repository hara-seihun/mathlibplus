...
namespace MathlibPlus.Open.ResearchFormalization.R1144Claim30201
...
/-- Claim 30201: the complementary four-subset has the same translated label. -/
def complementaryLinesCarrySameLabels_claim30201 : Prop :=
  ∀ (π : Equiv.Perm C7), nonlinearDevelopmentPermutation π →
    ∀ F : Set (Set C7), (F = fanoA ∨ F = fanoB) →
      ∀ (B : Set C7) (δ : C7 → C7),
        supportedLine π F B → normalizedLabel π B δ →
        ∀ t s : C7,
          Set.image π
              (translateSet (translateSet (Set.univ \ B) t) s) =
            translateSet
              (Set.image π (translateSet (Set.univ \ B) t))
              (translatedLabel π B δ t s)


end

end MathlibPlus.Open.ResearchFormalization.R1144Claim30201
