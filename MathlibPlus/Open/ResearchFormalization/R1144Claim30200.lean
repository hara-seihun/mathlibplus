...
namespace MathlibPlus.Open.ResearchFormalization.R1144Claim30200
...
/-- Claim 30200: labels on translated supported lines are the relative
    derivatives delta_t(s)=delta(t+s)-delta(t). -/
def translatedLabelsAreRelativeDerivatives_claim30200 : Prop :=
  ∀ (π : Equiv.Perm C7), nonlinearDevelopmentPermutation π →
    ∀ F : Set (Set C7), (F = fanoA ∨ F = fanoB) →
      ∀ (B : Set C7) (δ : C7 → C7),
        supportedLine π F B → normalizedLabel π B δ →
        ∀ t s : C7, labelRelation π B δ t s


end

end MathlibPlus.Open.ResearchFormalization.R1144Claim30200
