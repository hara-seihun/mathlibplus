...
namespace MathlibPlus.Open.ResearchFormalization.R1202Claim41963
...
/-- The nonlinear shear from the admitted affine-plane construction. -/
private def nonlinearShear (p : ℕ) [NeZero p] : Equiv.Perm (Plane p) :=
  ((Equiv.sigmaEquivProd (ZMod p) (ZMod p)).symm.trans
    (Equiv.sigmaCongrRight
      (fun x : ZMod p =>
        Equiv.addRight (Nat.choose x.val 2 : ZMod p)))).trans
    (Equiv.sigmaEquivProd (ZMod p) (ZMod p))

private def conjugates
    {α : Type*} (f : Equiv.Perm α)
    (P Q : Subgroup (Equiv.Perm α)) : Prop :=
  Set.image (fun g : Equiv.Perm α => f * g * f⁻¹) (P : Set (Equiv.Perm α)) =
    (Q : Set (Equiv.Perm α))
...
private def directProductScope {p : ℕ} [NeZero p]
    (P Q : Subgroup (Equiv.Perm (Plane p)))
    (Pcopy Qcopy : Subgroup (Equiv.Perm (Point p)))
    (Pkernel Qkernel : Subgroup (Equiv.Perm (Point p))) : Prop :=
  sixBlockSystem p ∧
...
private def commonBlockAndQuotientFacts {p : ℕ} [NeZero p]
    (Pcopy Qcopy : Subgroup (Equiv.Perm (Point p))) : Prop :=
  sixBlockSystem p ∧
    preservesBlocks Pcopy ∧
    preservesBlocks Qcopy ∧
    quotientImage Pcopy = (leftRegularSubgroup : Set (Equiv.Perm SixLabels)) ∧
...
/-- Claim 41963: the explicit nonlinear shear conjugates the two affine
rank-two groups.  Its lift to the common six-block direct product conjugates
those regular copies, while the same block and literal-quotient data do not
force equality of the two rank-two kernels. -/
def nonlinearShearScope_claim41963 : Prop :=
  ∀ (p : ℕ) (hp : Nat.Prime p), Odd p →
    ...
    let f := nonlinearShear p
    let lift := Equiv.prodCongr f (Equiv.refl SixLabels)
    (∀ x y : ZMod p,
        f (x, y) = (x, y + (Nat.choose x.val 2 : ZMod p))) ∧
      conjugates f P Q ∧
      directProductScope P Q Pcopy Qcopy Pkernel Qkernel ∧
      conjugates lift Pcopy Qcopy ∧
      commonBlockAndQuotientFacts Pcopy Qcopy ∧
      Pkernel ≠ Qkernel ∧
      ¬ (commonBlockAndQuotientFacts Pcopy Qcopy → Pkernel = Qkernel)

end

end MathlibPlus.Open.ResearchFormalization.R1202Claim41963
