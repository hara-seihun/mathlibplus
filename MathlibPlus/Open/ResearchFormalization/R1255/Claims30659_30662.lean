...
namespace MathlibPlus.Open.ResearchFormalization.R1255

abbrev QuotientCoordinate (q : ℕ) := ZMod q
abbrev LayerCoordinate := ZMod 3
abbrev Point (N : Type*) (q : ℕ) := (QuotientCoordinate q × N) × LayerCoordinate

/-- The order-three power of the local kernel automorphism. -/
...
/-- The exact right-regular coordinate formula from the source. -/
...
def blockPartition {N : Type*} {q : ℕ} : Set (Set (Point N q)) :=
  Set.range (block (N := N) (q := q))
...
noncomputable def orientationMap {N : Type*} {q : ℕ}
    (ε : QuotientCoordinate q → Bool) : Equiv.Perm (Point N q) := by
  classical
  apply Equiv.ofBijective (orientationMapFun (N := N) (q := q) ε)
  have hinv : Function.Involutive
      (orientationMapFun (N := N) (q := q) ε) := by
    intro p
    by_cases he : ε p.1.1 <;> simp [orientationMapFun, he]
...
def transportedPartition {N : Type*} {q : ℕ}
    (ε : QuotientCoordinate q → Bool) : Set (Set (Point N q)) :=
  {Set.image (orientationMap ε) (globalLayer (N := N) (q := q) 0),
    Set.image (orientationMap ε) (globalLayer (N := N) (q := q) 1),
    Set.image (orientationMap ε) (globalLayer (N := N) (q := q) 2)}
...
def alignedBlockAndLocalData {N : Type*} [AddCommGroup N] [Fintype N]
    (q : ℕ) (ω : ZMod q) (R T : Subgroup (Equiv.Perm (Point N q)))
...
def mixedTransportedLayer {N : Type*} {q : ℕ}
    (ε : QuotientCoordinate q → Bool) : Prop :=
  ∃ b c : ZMod q,
    ε b = false ∧ ε c = true ∧
      Set.image (orientationMap ε)
          (globalLayer (N := N) (q := q) 1 ∩ block b) =
        globalLayer (N := N) (q := q) 1 ∩ block b ∧
      Set.image (orientationMap ε)
          (globalLayer (N := N) (q := q) 1 ∩ block c) =
        globalLayer (N := N) (q := q) 2 ∩ block c

/-- Claim 30659: the derived-orbit characteristic partition transports by the
 exact orientation map, with the two constant codes preserving its unordered
 form. -/
def characteristicPartitionOfTEpsilon_claim30659 : Prop :=
...
/-- Claim 30660: exactly the constant codes preserve the characteristic
partition, and the code counts are 2^q, 2, and 2^q-2. -/
def constantCodePartitionCriterion_claim30660 : Prop :=
  ∀ {N : Type*} [AddCommGroup N] [Fintype N]
...
/-- Claim 30661: the same block/quotient/local data can coexist with a
 different characteristic partition for every nonconstant code. -/
def alignedDataDoesNotRecoverCharacteristicPartition_claim30661 : Prop :=
  ∀ {N : Type*} [AddCommGroup N] [Fintype N]
    (q : ℕ) (ω : ZMod q) (θ : N ≃+ N)
    (R : Subgroup (Equiv.Perm (Point N q))),
    partitionContext q ω θ R →
      ∀ ε : QuotientCoordinate q → Bool, ¬ constantCode ε →
        ...
        alignedBlockAndLocalData q ω R T ε ∧
...
/-- Claim 30662: the four exact controls retain their kernels, degrees, code
counts, constant counts, and breaker counts. -/
def exhaustiveNormalizedCodeControls_claim30662 : Prop :=
...
end MathlibPlus.Open.ResearchFormalization.R1255
