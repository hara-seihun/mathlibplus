...
namespace MathlibPlus.Open.ResearchFormalization.R1808Claim32486
...
def quotientConjugatesCopies
    (R T : Subgroup (Equiv.Perm Vertex))
    (h : Base ≃ₗ[ZMod 3] Base) : Prop :=
  ∀ p : Equiv.Perm Base,
    p ∈ inducedBaseCopy T ↔
      ∃ r ∈ inducedBaseCopy R,
        p = h.toEquiv * r * h.toEquiv.symm

def colorPreservingQuotientAutomorphism
    (c : Base → Fin 5) (h : Base ≃ₗ[ZMod 3] Base) : Prop :=
  ∀ u : Base, c (h u) = c u

def diagonalLift (h : Base ≃ₗ[ZMod 3] Base) : Vertex → Vertex :=
  fun p => (p.1, h p.2)

def diagonalGraphAutomorphism
    (S : Set Vertex) (h : Base ≃ₗ[ZMod 3] Base) : Prop :=
  ∀ x y : Vertex,
    cayleyAdj S x y ↔
      cayleyAdj S (diagonalLift h x) (diagonalLift h y)

def diagonalFiberMatrixPreservation
    (S : Set Vertex) (h : Base ≃ₗ[ZMod 3] Base) : Prop :=
  ∀ u v : Base,
    fiberRelation S (h u) (h v) = fiberRelation S u v

/-- Claim 32486: a color-preserving linear quotient transporter that
conjugates the two actual quotient copies has the literal diagonal lift as a
graph automorphism, with preservation of every four-point fiber matrix. -/
def quotientTransporterLiftsDiagonally_claim32486 : Prop :=
  ∀ (S : Set Vertex)
    (R T : Subgroup (Equiv.Perm Vertex))
    (f : Equiv.Perm Vertex)
    (π : Base → Equiv.Perm Fiber)
    ...
    quotientContext S R T f π q d →
      ∀ c : Base → Fin 5,
        quotientColorData S R T c →
          ∃ h : Base ≃ₗ[ZMod 3] Base,
            quotientConjugatesCopies R T h ∧
              colorPreservingQuotientAutomorphism c h ∧
                diagonalGraphAutomorphism S h ∧
                  diagonalFiberMatrixPreservation S h

end
end MathlibPlus.Open.ResearchFormalization.R1808Claim32486
