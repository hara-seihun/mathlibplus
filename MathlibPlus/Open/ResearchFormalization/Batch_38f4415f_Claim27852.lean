...
namespace MathlibPlus.Open.ResearchFormalization.Batch_38f4415f.Claim27852
...
private def exceptionType (S : Finset G72) : Prop :=
  ∃ U : Finset G72,
    connectionSet12 U ∧ ambientException U ∧
...
private def exceptionRepresentatives : Prop :=
  ∃ R : Finset (Finset G72),
    ...
    (∀ S, connectionSet12 S →
      ¬(partitionBreaking S ∧ nonlinearBase S)) ∧
    (∀ T, T ∈ R → connectionSet12 T ∧ exceptionType T) ∧
    (∀ S, connectionSet12 S → exceptionType S →
      ∃! T, T ∈ R ∧ connectionSet12 T ∧ exceptionType T ∧
        MathlibPlus.Open.Research.CIAtlas.graphEquivalent72 S T)

/-- The 1,997 valency-12 graph types in the two ambient recognition
    exception classes have one Aut(G)-presentation orbit in every graph fiber. -/
def claim27852 : Prop :=
  exceptionRepresentatives ∧
    ∀ S T : Finset G72,
      connectionSet12 S → connectionSet12 T → exceptionType S →
      MathlibPlus.Open.Research.CIAtlas.graphEquivalent72 S T →
      MathlibPlus.Open.Research.CIAtlas.autEquivalent72 S T

end MathlibPlus.Open.ResearchFormalization.Batch_38f4415f.Claim27852
