...
namespace MathlibPlus.Open.ResearchFormalization.WeightedSharedSelectorClaim51574
...
/-- Claim 51574: in the no-dominant case, the shared-first policy has the
exact variance and area formulas in the admitted statement. -/
def claim51574 : Prop :=
  ∀ (n : ℕ) (weights : Fin n → ℝ) (a : ℝ)
      (p : Equiv.Perm (Fin n)),
    (∀ i, 0 ≤ weights i) →
    (∑ i, weights i = 1) →
    maxWitness weights a →
    a ≤ 1 / 2 →
    descending weights p →
    let F : BoolCube (n + 2) → ℝ := selectorTarget n weights
    let s₂ : ℝ := ∑ i, weights i ^ 2
    let L : ℝ := tailArea weights p
    (∀ ω, ω 0 = ω 1 → F ω = boolSign (ω 0)) ∧
      expectedVariance F (∅ : Finset (Fin (n + 2))) =
        (1 + s₂) / 2 ∧
      expectedVariance F ({(0 : Fin (n + 2))} : Finset (Fin (n + 2))) =
        (1 + 2 * s₂) / 4 ∧
      (s₂ ≤ 1 / 2) ∧
      (L ≤ 1) ∧
      (∀ ω, (sharedQueries p ω).Nodup) ∧
      policyArea F (sharedQueries p) =
        3 / 4 + s₂ + L / 2 ∧
      policyArea F (sharedQueries p) ≤ 7 / 4 ∧
      policyArea F (sharedQueries p) < 2


end

end MathlibPlus.Open.ResearchFormalization.WeightedSharedSelectorClaim51574
