...
namespace MathlibPlus.Open.ResearchFormalization.O0356.Claim15677
...
end MathlibPlus.Open.ResearchFormalization.O0356.Claim15677
...
/-- Claim 15677: on the RH branch, the first critical conjugate pair gives the
raw exterior-square asymptotic at the matching Poisson shell, with a strictly
negative coefficient and eventual strict negativity. -/
def claim15677_exactFirstPairExteriorSquareAsymptotic : Prop :=
  ∀ (S_f : ℕ → ℂ) (ρ₁ : ℂ) (γ₁ : ℝ),
    IsHadamardLiCoefficientSequence S_f →
      RiemannHypothesis →
        FirstPositiveNontrivialZero ρ₁ γ₁ →
          ...
          let d₁ := FirstSlope ρ₁
          let H : ℝ → ℂ := fun x =>
            PoissonTuránSquare S_f (FirstShellIndex ρ₁ x) x
          (0 < d₁ ∧
              b.im ≠ 0 ∧
              (b - starRingEnd ℂ b) ^ 2 =
                ((-4 * b.im ^ 2 : ℝ) : ℂ) ∧
              (b - starRingEnd ℂ b) ^ 2 ≠ 0) ∧
            RelativeExponentialAsymptotic H
              (fun x : ℝ => FirstPairLeading ρ₁ x) ∧
            EventuallyStrictlyNegative H
