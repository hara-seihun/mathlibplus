...
namespace MathlibPlus.Open.ResearchFormalization.R2056C35884
...
/-- Claim 35884: the actual separable nine-point map is injective, has the
stated signed chain displacements with no cross-family displacement, and its
four actual convex-hull cells form an embedded face-to-face disk. -/
def claim35884_embeddingAndFaceToFace : Prop :=
  embeddedFaceToFaceDisk ∧
    (∀ i j : Fin 3, i < j →
      0 < (U j - U i).1 ∧ (U j - U i).2 ≤ 0) ∧
    (∀ i j : Fin 3, i < j →
      0 ≤ (V j - V i).1 ∧ 0 < (V j - V i).2) ∧
...
end MathlibPlus.Open.ResearchFormalization.R2056C35884
