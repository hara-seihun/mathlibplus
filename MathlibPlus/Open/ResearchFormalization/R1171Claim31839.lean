...
namespace MathlibPlus.Open.ResearchFormalization.R1171Claim31839
...
def actualImageException (g : F5 → F5) : Prop :=
  ¬ actualImageConjugate (lift g)

/-- Claim 31839: the normalized functions have exactly the six reviewed
    affine-relabeling classes, with their displayed representatives, orbit
    sizes, derivative-module ranks, and exactly the five nontrivial
    actual-image exception classes. -/
def claim31839 : Prop :=
  Set.ncard {g : F5 → F5 | normalizedFunction g} = 625 ∧
    ...
    (∀ i : Fin 6,
      Module.finrank F5
        ...
        classModuleRank i) ∧
    (∀ i : Fin 6,
      actualImageException (classRepresentative i) ↔ i ≠ 0)

end

end MathlibPlus.Open.ResearchFormalization.R1171Claim31839
