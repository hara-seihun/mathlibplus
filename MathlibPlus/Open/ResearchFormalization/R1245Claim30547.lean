...
namespace MathlibPlus.Open.ResearchFormalization.R1245Claim30547
...
def zeroLayerInverseClosed {n : ℕ} (S : Set (Q4n n)) : Prop :=
  ∀ x : Cn n, (x, 0) ∈ S → (-x, 0) ∈ S
...
def alphaFunction {n : ℕ} (t : Cn n) : Q4n n → Q4n n :=
  fun z =>
    (-z.1 + if z.2.val % 2 = 1 then t else 0, z.2)

def alphaGroupAutomorphism {n : ℕ} (t : Cn n) : Prop :=
  Function.Bijective (alphaFunction t) ∧
    ∀ x y : Q4n n,
      alphaFunction t (fourLayerMul x y) =
        fourLayerMul (alphaFunction t x) (alphaFunction t y)

def quietCoordinateEquation {n : ℕ}
    (t : Cn n) (τ : Fin 4 → Cn n) (p : ℕ) (hpn : p ∣ n) : Prop :=
  primeCoordinate n p hpn t =
    2 * primeCoordinate n p hpn (τ 2) -
      primeCoordinate n p hpn (τ 1)

def activeCoordinateEquation {n : ℕ}
    (t : Cn n) (p : ℕ) (hpn : p ∣ n) : Prop :=
  primeCoordinate n p hpn t = 0

/-- Claim 30547: the quiet and active CRT coordinates assemble one element
whose explicit inversion--translation map is a group automorphism; it shadows
f on S, while the active nonzero sections are saturated and the zero layer is
    ...
    ∀ (lambdas : Fin 4 → (Cn n)ˣ) (τ : Fin 4 → Cn n)
      ...
      ∃ t : Cn n,
        (∀ (p : ℕ) (hp : Nat.Prime p) (hpn : p ∣ n),
          quietAt f π τ p hpn →
            quietCoordinateEquation t τ p hpn) ∧
        (∀ (p : ℕ) (hp : Nat.Prime p) (hpn : p ∣ n),
          (scalarActiveAt f p ∨ translationActiveAt f π τ p hpn) →
            activeCoordinateEquation t p hpn) ∧
        alphaGroupAutomorphism t ∧
        everyNonzeroSectionPeriodic A S ∧
        zeroLayerInverseClosed S ∧
        Set.image (alphaFunction t) S = Set.image f S

end

end MathlibPlus.Open.ResearchFormalization.R1245Claim30547
