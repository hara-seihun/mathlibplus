...
namespace MathlibPlus.Open.ResearchFormalization.R0554Claim26150
...
/-- Claim 26150: outside traces of an independent triple are nonempty and
    pairwise intersecting, and disjoint traces give an induced forest even
    when the two outside vertices may be adjacent. -/
def independentTripleTraceIntersection_claim26150 : Prop :=
  ∀ {V : Type*} [Fintype V] [DecidableEq V]
    (G : SimpleGraph V) (T : Finset V),
    cyclicFive G →
    T.card = 3 →
    independentWithin G (T : Set V) →
    (∀ u : V, u ∉ T →
      (traceSet G (T : Set V) u).Nonempty) ∧
    (∀ u v : V, u ∉ T → v ∉ T → u ≠ v →
      (traceSet G (T : Set V) u ∩ traceSet G (T : Set V) v).Nonempty) ∧
    (∀ u v : V, u ∉ T → v ∉ T → u ≠ v →
      traceSet G (T : Set V) u ∩ traceSet G (T : Set V) v = ∅ →
      inducedForestOn G (insert u (insert v (T : Set V))))


end

end MathlibPlus.Open.ResearchFormalization.R0554Claim26150
