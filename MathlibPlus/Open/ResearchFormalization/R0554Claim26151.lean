...
namespace MathlibPlus.Open.ResearchFormalization.R0554Claim26151
...
/-- Claim 26151: the cyclic-five equality hypotheses forbid an independent
    triple at every order at least eight. -/
def equalityNoIndependentTriple_claim26151 : Prop :=
  ∀ n : ℕ, 8 ≤ n → ∀ H : SimpleGraph (Fin n),
    cyclicFive H →
    graphEdgeCount H = equalityEdgeCount n →
    ¬ ∃ T : Finset (Fin n),
      T.card = 3 ∧ independentWithin H (T : Set (Fin n))


end

end MathlibPlus.Open.ResearchFormalization.R0554Claim26151
