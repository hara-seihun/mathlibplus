...
root-inclusive posterior-variance areas of denominator-ten depth-at-most-two
laws with four pairwise-distinct semantic tables are `1151 / 800`, `1133 / 800`,
`67 / 50`, `137 / 100`, and `23 / 16` in the respective multiplicity strata
`4+4+1+1`, `4+3+2+1`, `4+2+2+2`, `3+3+3+1`, and `3+3+2+2`.
-/
def denominatorTenAllLightSupportFourDepthTwoFiveCubeAreaMaxima : Prop :=
  let Ω := Fin 5 → Bool
  ...
  let target : Target := fun trees weights x =>
    (∑ i : Fin 4, weights i * evalTree (trees i) x) / 10
  let Attains : (Fin 4 → ℚ) → ℚ → Prop := fun weights bound =>
    (∀ trees : Fin 4 → Tree, Distinct trees →
      ∃ q0 q1 q2 q3 q4, Fresh q0 q1 q2 q3 q4 ∧
        PolicyArea (target trees weights) q0 q1 q2 q3 q4 ≤ bound) ∧
      ∃ trees : Fin 4 → Tree, Distinct trees ∧
        ∀ q0 q1 q2 q3 q4, Fresh q0 q1 q2 q3 q4 →
          bound ≤ PolicyArea (target trees weights) q0 q1 q2 q3 q4
  Attains ![4, 4, 1, 1] (1151 / 800) ∧
    Attains ![4, 3, 2, 1] (1133 / 800) ∧
    Attains ![4, 2, 2, 2] (67 / 50) ∧
    Attains ![3, 3, 3, 1] (137 / 100) ∧
    Attains ![3, 3, 2, 2] (23 / 16)
