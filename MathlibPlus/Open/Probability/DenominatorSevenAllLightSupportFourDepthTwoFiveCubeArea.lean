...
root-inclusive posterior-variance areas of denominator-seven depth-at-most-two
laws with four pairwise-distinct semantic tables are `1073 / 784` in the
multiplicity stratum `3+2+1+1` and `537 / 392` in the stratum `2+2+2+1`.
-/
def denominatorSevenAllLightSupportFourDepthTwoFiveCubeAreaMaxima : Prop :=
  let Ω := Fin 5 → Bool
  ...
  let target3211 : (Fin 4 → Tree) → Ω → ℚ := fun trees x =>
    (3 * evalTree (trees 0) x + 2 * evalTree (trees 1) x +
      evalTree (trees 2) x + evalTree (trees 3) x) / 7
  let target2221 : (Fin 4 → Tree) → Ω → ℚ := fun trees x =>
    (evalTree (trees 0) x + 2 * evalTree (trees 1) x +
      2 * evalTree (trees 2) x + 2 * evalTree (trees 3) x) / 7
  (∀ trees : Fin 4 → Tree, Distinct trees →
    ∃ q0 q1 q2 q3, Fresh q0 q1 q2 q3 ∧
      PolicyArea (target3211 trees) q0 q1 q2 q3 ≤ 1073 / 784) ∧
    (∃ trees : Fin 4 → Tree, Distinct trees ∧
      ∀ q0 q1 q2 q3, Fresh q0 q1 q2 q3 →
        1073 / 784 ≤ PolicyArea (target3211 trees) q0 q1 q2 q3) ∧
    (∀ trees : Fin 4 → Tree, Distinct trees →
      ∃ q0 q1 q2 q3, Fresh q0 q1 q2 q3 ∧
        PolicyArea (target2221 trees) q0 q1 q2 q3 ≤ 537 / 392) ∧
    ∃ trees : Fin 4 → Tree, Distinct trees ∧
      ∀ q0 q1 q2 q3, Fresh q0 q1 q2 q3 →
        537 / 392 ≤ PolicyArea (target2221 trees) q0 q1 q2 q3
