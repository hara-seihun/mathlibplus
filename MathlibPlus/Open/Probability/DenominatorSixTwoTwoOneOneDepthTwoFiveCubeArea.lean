...
/--
On the uniform five-coordinate Boolean cube, among averages of six
depth-at-most-two Boolean components with four pairwise-distinct semantic
tables and multiplicities `2+2+1+1`, the exact maximum optimal
root-inclusive posterior-variance area is `23 / 16`.
-/
def denominatorSixTwoTwoOneOneDepthTwoFiveCubeAreaMaximum : Prop :=
  let Ω := Fin 5 → Bool
  ...
  let target : (Fin 4 → Tree) → Ω → ℚ := fun trees x =>
    (2 * evalTree (trees 0) x + 2 * evalTree (trees 1) x +
      evalTree (trees 2) x + evalTree (trees 3) x) / 6
  (∀ trees : Fin 4 → Tree, Distinct trees →
    ∃ q0 q1 q2 q3, Fresh q0 q1 q2 q3 ∧
      PolicyArea (target trees) q0 q1 q2 q3 ≤ 23 / 16) ∧
    ∃ trees : Fin 4 → Tree, Distinct trees ∧
      ∀ q0 q1 q2 q3, Fresh q0 q1 q2 q3 →
        23 / 16 ≤ PolicyArea (target trees) q0 q1 q2 q3
