...
/--
Exact denominator-nine all-light depth-two area maxima on the uniform five-sign
cube: `3+3+2+1` has maximum `101/72`, and `3+2+2+2` has maximum `112/81`.
-/
def denominatorNineAllLightSupportFourDepthTwoFiveCubeAreaMaxima : Prop :=
  let Ω := Fin 5 → Bool
  ...
  let Fresh : Fin 5 → (Bool → Fin 5) → (Bool → Bool → Fin 5) →
      (Bool → Bool → Bool → Fin 5) →
      (Bool → Bool → Bool → Bool → Fin 5) → Prop := fun q0 q1 q2 q3 q4 =>
    (∀ a, q1 a ≠ q0) ∧
      (∀ a b, q2 a b ≠ q0 ∧ q2 a b ≠ q1 a) ∧
      (∀ a b c, q3 a b c ≠ q0 ∧ q3 a b c ≠ q1 a ∧ q3 a b c ≠ q2 a b) ∧
      ∀ a b c d, q4 a b c d ≠ q0 ∧ q4 a b c d ≠ q1 a ∧
  ...
  let PolicyArea : (Ω → ℚ) → Fin 5 → (Bool → Fin 5) →
    ...
    fun g q0 q1 q2 q3 q4 =>
      ...
      let var4 := fun a b c d : Bool =>
        (∑ x : Ω,
          if x q0 = a ∧ x (q1 a) = b ∧ x (q2 a b) = c ∧
              x (q3 a b c) = d then (g x) ^ 2 else 0) / 2 - (mean4 a b c d) ^ 2
      var0 + (∑ a : Bool, var1 a) / 2 +
        (∑ a : Bool, ∑ b : Bool, var2 a b) / 4 +
        (∑ a : Bool, ∑ b : Bool, ∑ c : Bool, var3 a b c) / 8 +
        (∑ a : Bool, ∑ b : Bool, ∑ c : Bool, ∑ d : Bool, var4 a b c d) / 16
  let target3321 : (Fin 4 → Tree) → Ω → ℚ := fun trees x =>
    (3 * evalTree (trees 0) x + 3 * evalTree (trees 1) x +
      2 * evalTree (trees 2) x + evalTree (trees 3) x) / 9
  let target3222 : (Fin 4 → Tree) → Ω → ℚ := fun trees x =>
    (3 * evalTree (trees 0) x + 2 * evalTree (trees 1) x +
      2 * evalTree (trees 2) x + 2 * evalTree (trees 3) x) / 9
  (∀ trees : Fin 4 → Tree, Distinct trees →
    ∃ q0 q1 q2 q3 q4, Fresh q0 q1 q2 q3 q4 ∧
      PolicyArea (target3321 trees) q0 q1 q2 q3 q4 ≤ 101 / 72) ∧
    (∃ trees : Fin 4 → Tree, Distinct trees ∧
      ∀ q0 q1 q2 q3 q4, Fresh q0 q1 q2 q3 q4 →
        101 / 72 ≤ PolicyArea (target3321 trees) q0 q1 q2 q3 q4) ∧
    (∀ trees : Fin 4 → Tree, Distinct trees →
      ∃ q0 q1 q2 q3 q4, Fresh q0 q1 q2 q3 q4 ∧
        PolicyArea (target3222 trees) q0 q1 q2 q3 q4 ≤ 112 / 81) ∧
    ∃ trees : Fin 4 → Tree, Distinct trees ∧
      ∀ q0 q1 q2 q3 q4, Fresh q0 q1 q2 q3 q4 →
        112 / 81 ≤ PolicyArea (target3222 trees) q0 q1 q2 q3 q4
