...
/-- Exact finite pairing-existence screen for the next six positive-weight rows of three-sign depth-two
four-atom laws.  The carrier is deliberately local to this finite registry
statement; it records the complete Bellman-area comparison, not an external
verifier digest. -/
noncomputable def fourAtomPairingWeightSevenThroughTwelve : Prop :=
  let Table := Fin 8 → ℚ
