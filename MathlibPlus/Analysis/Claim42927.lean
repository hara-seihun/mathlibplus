...
namespace MathlibPlus.Analysis.Claim42927
...
private lemma intervalAverage_representation
    (p : ℕ) (hp : Nat.Prime p) {y : ℝ} (hy : 1 < y) :
    let L : ℝ := Real.log (p : ℝ)
    Real.log (Real.log (y + L) / Real.log y) / L =
      (1 / L) * ∫ v in y..y + L, (1 / (v * Real.log v)) := by
  dsimp
  have hL : 0 < Real.log (p : ℝ) := hp.log_pos
  have hy' : 0 < y := by linarith
  have hyL : 1 < y + Real.log (p : ℝ) := by linarith
  have hlogy : 0 < Real.log y := Real.log_pos hy
  have hlogyL : 0 < Real.log (y + Real.log (p : ℝ)) :=
    Real.log_pos hyL
  have hint :
      (∫ v in y..y + Real.log (p : ℝ), 1 / (v * Real.log v)) =
        Real.log (Real.log (y + Real.log (p : ℝ))) - Real.log (Real.log y) := by
    convert (integral_inv_div_log hy hyL) using 1
    apply intervalIntegral.integral_congr
    intro v hv
    simp [div_eq_mul_inv]
    ring
  rw [hint, Real.log_div (ne_of_gt hlogyL) (ne_of_gt hlogy)]
  field_simp [ne_of_gt hL]

/-- Claim 42927: the logarithmic interval-average representation and strict
squeeze for a prime `p`. -/
theorem intervalAverageRepresentationAndStrictSqueeze :
  ∀ (p : ℕ), Nat.Prime p → ∀ y : ℝ, y > 1 →
    let L : ℝ := Real.log (p : ℝ)
    let η : ℝ → ℝ := fun x ↦
      Real.log (Real.log (x + L) / Real.log x) / L
    (η y = (1 / L) * ∫ v in y..y + L, (1 / (v * Real.log v))) ∧
      (y - L > 1 →
        η y < 1 / (y * Real.log y) ∧
  ...
  dsimp
  have hL : 0 < Real.log (p : ℝ) := hp.log_pos
  have hrepr := intervalAverage_representation p hp hy
  constructor
  · exact hrepr
  · intro hprev
    have hleft := intervalAverage_lt_left y (Real.log (p : ℝ)) hy hL
    have hright := intervalAverage_gt_right (y - Real.log (p : ℝ))
        (Real.log (p : ℝ)) hprev hL
    constructor
    · rw [hrepr]
      exact hleft
    · have hreprPrev := intervalAverage_representation p hp hprev
      rw [hreprPrev]
      simpa [sub_add_cancel] using hright

end MathlibPlus.Analysis.Claim42927
